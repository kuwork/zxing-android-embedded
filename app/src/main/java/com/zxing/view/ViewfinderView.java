...
public final class ViewfinderView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		Rect frame2 = CameraManager.get().getFramingRectInPreview();
		if (frame == null) {
			return;
		}
		...
		if (resultBitmap != null) {
			...
			// textPaint
			// .setTextSize(getResources().getDimension(R.dimen.dim_20dp));
			// textPaint.setColor(Color.WHITE);
			// canvas.drawText(getResources().getString(R.string.tip_capture),
			// middle - 112, frame.bottom + 60, textPaint);
			...
			if (laserLinePortrait) {
				...
				if ((i += 5) < frame.bottom - frame.top) {
					...
					canvas.clipRect(frame.left + 2, frame.top - 3 + i,
							frame.right - 1, frame.top + 3 + i);
